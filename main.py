...
from schemas import (JobFilters, JobFiltersModel, MfaBodyModel, SalesOrder,
                     SizeItem)
...
async def _click_and_wait_domcontent(page: Page, locator: Locator, timeout: int = 15000):
    try:
...
async def clean_not_order_yet_tags(
    ...
    async def tag_cleanup_on_order_page(p: Page) -> None:
        ...
        # open dropdown (caret first, fallback to combobox)
...
async def checkout(orders: List["SalesOrder"], max_concurrency: int = 3):
    ctx = await get_ctx()

    async def goto_home(page: Page):
        await page.goto(URL_SANMAR, wait_until="domcontentloaded")

    async def build_size_inputs_by_warehouse(page: Page) -> Dict[str, List[Tuple[str, Locator, int]]]:
        await page.wait_for_selector("table.table-inventory.table-inventory-next", timeout=15000)
        await page.wait_for_selector("table.table-inventory.table-inventory-next thead th.size-header", timeout=15000)
        ...
        for t_idx in range(tcount):
            ...
            for r_idx in range(rcount):
                ...
                for h_idx, raw_label in enumerate(header_labels):
                    ...
                    size_to_entries.setdefault(size_key, []).append((wh_name, input_field, available_qty))

        # One-size aliasing if there’s only one size column total
    ...
    async def add_requested_sizes(page: Page, sizes: List["SizeItem"]) -> Tuple[List[str], bool]:
        try:
        ...
        for s in sizes:
            ...
            # Find the first matching size key present in the table
            size_key = next((c for c in candidates if c in size_entries), None)
            if not size_key:
                # No matching column found; skip silently (or add to OOS if you prefer)
                continue

            # Iterate warehouses in the DOM order (Primary first, then others)
            for wh_name, input_field, available_qty in size_entries[size_key]:
                if remaining <= 0:
                    break

                # Skip disabled inputs or zero availability
                try:
                    if await input_field.is_disabled():
                        continue
                except Exception:
                    # Fallback if is_disabled not available in your Playwright version
                    try:
                ...
                try:
                    ...
                    await input_field.fill(str(to_take))
                    added_any = True
                    remaining -= to_take
                except Exception:
                    continue
            ...
            if remaining > 0:
                out_of_stock.append(f"{s.size} (short {remaining})")

        if added_any:
            add_to_cart_button = page.locator("button.btn.btn-primary.btn-add-to-basket").first
            await add_to_cart_button.wait_for(state="visible", timeout=7000)
            await add_to_cart_button.click()
            await page.wait_for_timeout(500)

        return out_of_stock, added_any
    ...
    sem = asyncio.Semaphore(max_concurrency)
    tasks = [asyncio.create_task(process_order(order, sem)) for order in orders]
...
async def login():
    ...
    try:
        ...
        await page.goto(
            next_url, wait_until="domcontentloaded", timeout=TIMEOUT_MS_DEFAULT
        )

        # Ensure sign-in form fields are visible
        await page.locator("#email-input").wait_for(
            state="visible", timeout=TIMEOUT_MS_DEFAULT
        )
...
@app.get("/sanmar-login")
async def sanmar_login():
    ...
    try:
        ...
        await page.goto(
            URL_SANMAR, wait_until="domcontentloaded", timeout=TIMEOUT_MS_DEFAULT
        )
...
async def add_to_cart(orders: List[SalesOrder]):
    result = await checkout(orders)
    return JSONResponse(content={"result": result}, status_code=200)
